...
class twitter_bot(object):
    ...
    def perform_train_test_split(df):
        ...
        X_train, y_train = train, train.iloc[:, -1]  # Replace ix with iloc
        X_test, y_test = test, test.iloc[:, -1]  # Replace ix with iloc
    ...
    def bot_prediction_algorithm(df):
        ...
        train_df['followers_count'] = train_df.followers_count.apply(lambda x: 0 if pd.isna(x) or x == 'None' else int(x))
        train_df['friends_count'] = train_df.friends_count.apply(lambda x: 0 if pd.isna(x) or x == 'None' else int(x))
